...
(a 1-D matching graph — the simplest system with a real threshold):
...
```
qec/
  codes/       circuit constructors (repetition code, rotated surface code)
  noise/       circuit-level noise models
  decoders/    uniform decoder interface (MWPM / union-find / BP+OSD)
  harness/     Monte-Carlo sampling, threshold fitting, latency benchmarking
experiments/   one runnable script per result figure
tests/         fast smoke tests (~1 s)
figures/       generated by the experiments; regenerable
```

Engineering notes worth a look:

- `qec/harness/threshold.py` — the code-agnostic threshold harness: any
  `(distance, rounds, p) -> stim.Circuit` callable plugs in; includes the
  finite-size scaling estimator (`estimate_threshold`), whose correctness is
  pinned by a test that recovers a known threshold from synthetic data.
- `qec/decoders/wrappers.py` — the uniform decoder layer and the documented
  parameter choices behind the benchmark's fairness claims.
- `qec/harness/latency.py` — the timing methodology, stated explicitly.
